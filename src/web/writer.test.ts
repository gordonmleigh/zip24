  ...
  describe("class ZipWriter", () => {
    describe(".fromWritableStream()", () => {
      ...
      it("produces the correct data", async () => {
        ...
        const close = mock.fn<Required<UnderlyingSink<Uint8Array>>["close"]>();

        const write = mock.fn<Required<UnderlyingSink<Uint8Array>>["write"]>(
          (chunk) => {
            chunks.push(chunk);
          },
        );
    ...
    it("writes directly to a sink if given", async () => {
      const writtenChunks: Uint8Array[] = [];
      const close = mock.fn<ByteSink["close"]>();

      // eslint-disable-next-line @typescript-eslint/require-await
      const write = mock.fn<ByteSink["write"]>(async (chunk) => {
      ...
      assert.strictEqual(close.mock.callCount(), 1);
      assertBufferEqual(data(...writtenChunks), expected);
    });

    it("throws on attempts to read when initialized with sink", async () => {
      const close = mock.fn<ByteSink["close"]>();
      const write = mock.fn<ByteSink["write"]>(() => Promise.resolve());
      ...
      await assert.rejects(
        () => writer[Symbol.asyncIterator]().next(),
