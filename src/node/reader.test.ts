...
describe("node/reader", () => {
  describe("ZipReader", () => {
    ...
    it("can read a Zip64 with a large extensible data sector", async () => {
      // 12 MB file with 100 files of 100 kB each + roughly 2 MB central dir
      const data = await buffer(
        generateZip({
          fileCount: 5,
          fileSize: 100,
          zip64: true,
          // pad out the zip64 eocdr to force another chunk to be read
          zip64ExtensibleDataLength: 1024 ** 2,
        }),
      );
      const reader = new ZipReader(
        randomAccessReaderFromBuffer(data),
